...
const IssuerSchema = new mongoose.Schema({
  ...
  login_logs: [{ ip: String, at: Date, user_agent: String }],
  last_login: Date,
  ...
  file_size: String,
  upload_date: Date,
  qr_code: { format: String, data: String },
...
app.post('/api/login', async (req, res) => {
  ...
  if (!issuer) {
    // first time – create exactly like your sample
    issuer = new Issuer({
      issuer_id: 'issuer_' + Date.now(),
      name,
      email,
      password_hash: hash,
      ...
      login_logs: [],
      login_attempts: 0,
      roles_permissions: ['issuer'],
      document_id: 'DOC_' + Date.now(),
      file_hash: 'sha256:placeholder',
      file_path: '/uploads/placeholder.pdf',
      file_size: '0 MB',
      upload_date: new Date(),
      qr_code: { format: 'png', data: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==' },
      watermark_key: 'wm_key_' + Date.now(),
    ...
    issuer.password_hash = hash;
    issuer.updated_at = new Date();
    issuer.updated_by = email;
  }
  ...
  issuer.last_login = new Date();
  issuer.last_login_ip = req.ip || '127.0.0.1';
  issuer.login_attempts = 0;
...
module.exports = app;   // or app.listen(4000) if you run standalone
