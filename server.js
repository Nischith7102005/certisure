...
const app = express();
app.use(cors());   // ← allow Vercel front-end
app.use(express.json());

const MONGO = 'mongodb://localhost:27017/CERTISURE';   // your local db
const JWT_SEC = process.env.JWT_SECRET || 'change-me-32-chars';
const COLL = 'ISSUER_DOCUMENT';

mongoose.connect(MONGO).then(()=>console.log('Local Mongo connected'));

const IssuerSchema = new mongoose.Schema({
  issuer_id:String, name:String, email:{type:String,unique:true}, password_hash:String,
  institution_name:String, is_verified:Boolean, sessions:[{token:String,created_at:Date,expires_at:Date}],
  login_logs:[{ip:String,at:Date,user_agent:String}], last_login:Date, last_login_ip:String,
  login_attempts:Number, roles_permissions:[String], document_id:String, file_hash:String,
  file_path:String, file_size:String, upload_date:Date, qr_code:{format:String,data:String},
  watermark_key:String, is_authenticated:Boolean, status:String, document_type:String,
  visibility:String, modification_logs:[], access_logs:[], version_history:[], metadata:[],
  download_count:Number, view_count:Number, last_accessed:Date, expires_at:Date,
  is_archived:Boolean, created_at:Date, updated_at:Date, created_by:String, updated_by:String
},{collection:COLL});

const Issuer = mongoose.model('Issuer', IssuerSchema);

/* ---- /api/login  (creates / updates issuer) ---- */
...
app.post('/api/logout', async (req,res)=>{
  ...
  const {uid} = jwt.verify(token,JWT_SEC);
...
app.listen(PORT,()=>console.log(`Local backend → http://localhost:${PORT}`));